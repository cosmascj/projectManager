...
import com.example.domoreeveryday.model.SelectedMembers

class CardMemberListItemsAdapter(private var context: Context,
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        if (holder is MyViewHolder) {
            ...
            if (position == list.size - 1) {
                holder.itemView.findViewById<TextView>(R.id.iv_add_member).visibility = View.VISIBLE
                holder.itemView.findViewById<ImageView>(R.id.iv_selected_member_image).visibility = View.GONE
            } else {
                holder.itemView.findViewById<TextView>(R.id.iv_add_member).visibility = View.GONE
